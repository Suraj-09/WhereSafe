...
public class FirestoreHelper<TeamMember> {
    ...
    private final FirestoreData firestoreData;
    ...
    public void getUser(String uid, FirestoreCallback firestoreCallback) {
        usersCollection.document(uid).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
    ...
    public void addBmeData(String uid, BmeData bmeData) {
        ...
        getSensorDataCollection(uid).add(bmeMap).addOnSuccessListener(documentReference -> Log.d(TAG, "addBmeData(): DocumentSnapshot written with ID: " + documentReference.getId())).addOnFailureListener(e -> Log.w(TAG, "addBmeData(): Error adding document", e));
    }

    public void getLatestPersonalSensorData(String uid, FirestoreCallback firestoreCallback) {
        getSensorDataCollection(uid).orderBy("timestamp", Query.Direction.DESCENDING).limit(1).get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
    ...
    public void getAllPersonalSensorData(String uid, FirestoreCallback firestoreCallback) {
        usersCollection.document(uid).collection(FirestoreConfig.COLLECTION_SENSOR_DATA).orderBy("timestamp", Query.Direction.DESCENDING).limit(20).get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                ArrayList<BmeData> bmeDataArrayList = new ArrayList<>();
                for (QueryDocumentSnapshot document : task.getResult()) {
                    bmeDataArrayList.add(new BmeData(document.getId(), document.getData()));
    ...
    public void getTeamMembers(String teamCode, FirestoreCallback firestoreCallback) {
        usersCollection.whereEqualTo("team_code", teamCode).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
    ...
    public void updateDeviceName(String uid, String newDeviceName) {
        usersCollection.document(uid).update("device_name", newDeviceName).addOnSuccessListener(aVoid -> Log.d(TAG, "Device name successfully updated")).addOnFailureListener(e -> Log.e(TAG, "Error updating device name", e));
    }
    ...
    public void addMember(String uid, String teamCode) {
        teamsCollection.document(teamCode).update("members", FieldValue.arrayUnion(usersCollection.document(uid))).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void unused) {
    ...
    public void updateUserLocation(double latitude, double longitude) {
        ...
        if (user != null) {
            ...
            documentReference.update("longitude", longitude).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void aVoid) {
